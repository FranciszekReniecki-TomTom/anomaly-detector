...
class EnginePlots {
    ...
    fun `test drawAnomalies`() {
        ...
        val tiles: Map<Long, List<TrafficTileHour>> = data
            .groupBy { it.id }

        println("Tiles size: ${tiles.size}")

        val trafficOutliers: List<TrafficTileHour> = tiles.map { (_, traffics) ->
            val speeds = traffics.map { it.traffic.speedKmH }.toDoubleArray()
            findWeeklyOutliers(speeds, 2.0)
                .map { traffics[it] }
        }.flatten().toList()

        println("Outliers size: ${trafficOutliers.size}")

        val outliers: Array<DoubleArray> = trafficOutliers
            .map { it.geoTime() }
            .toTypedArray()
...
//        drawKDistances(outliers)
        ...
        val trafficClusters: List<List<TrafficTileHour>> = findClusters(outliers, radius = 0.2, noise = false)
            .map { cluster ->
        ...
        val clustersWithIndex = trafficClusters
            .withIndex()
            .map { (index, cluster) ->
                cluster.map { it.geoTime() + index.toDouble() }
            }
            .flatten()
            .toTypedArray()
