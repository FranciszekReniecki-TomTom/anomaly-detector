...
internal fun drawAnomalies(anomalies: List<List<TrafficTileHour>>) {
  val points: Array<DoubleArray> = anomalies
    .withIndex()
    .map { (i, tthList) ->
      tthList
...
internal fun drawKDistances(points: Array<DoubleArray>) {
  ...
  val scatterPlot = ScatterPlot.of(
    distances.withIndex()
      .map { (i, d) -> doubleArrayOf(i.toDouble(), d) }
      .toTypedArray()
  )
...
internal fun testRadius(points: Array<DoubleArray>) {
  ...
  generateSequence(0.02) { it + 0.02 }
    ...
    .forEach { radius ->
      ...
      println("Cluster with $radius: ${clusters.k()} clusters [sizes: ${clusters.size().toList()}]")
    }
}
